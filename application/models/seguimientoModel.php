...
class SeguimientoModel extends CI_Model{
	...
	function martin548(){
		$tm = $this->db->query("select gva14.cuit, gva14.id
			from Diccionario_000245_014..gva14
			left join Diccionario_000245_014..[valores_agregados] on gva14.cuit = valores_agregados.cuit
			where isnull(valores_agregados.cuit, '') = '' 
			and isnull(gva14.cuit,'') <> '' 
			and gva14.TIPO_DOC = '80' and Diccionario_000245_014.dbo.fn_ValidarCUIT(gva14.cuit) = 1
			order by  gva14.id asc");
		...
		foreach ($registros as $fila) {
			...
			if($pagina === FALSE) { 
				...
				log_message('error', trim($file_headers[0]));
	        ...
	        $tm = $this->db->query("INSERT INTO 
	        Diccionario_000245_014.[dbo].[valores_agregados]([cuit],[contrato_social],[facturacion_estimada],[cantidad_empleados],[otros_datos],[interaccion],[tipoPersona],[estadoClave],[mesCierre],[codPostal],[idProvincia],[nombreProvincia],[localidad],[direccion],[categoria],[idActividad],[descActividad]".$fecha_modificados.")  
	        values 
	        (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?". $fecha_modificados_valor." )", 
	        array($cuit_bd, $contrato_social, $facturacion_estimada, $cantidad_empleados, $otros_datos, trim($interaccion), $tipoPersona, $estadoClave, $mesCierre, $codPostal, $idProvincia, $nombreProvincia, $localidad, $direccion, $categoria, $idActividad, $descActividad));
